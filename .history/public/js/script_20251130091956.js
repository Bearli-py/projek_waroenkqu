    ...
    */
    ...
    filterButtons.forEach(button => {
        ...
        button.addEventListener('click', function() {
            ...
            // 1. AMBIL KATEGORI dari data-filter button yang diklik
            const filterValue = this.getAttribute('data-filter');
            
            // 2. REMOVE class "active" dari semua button (reset)
            filterButtons.forEach(btn => btn.classList.remove('active'));
            
            // 3. ADD class "active" ke button yang diklik
            this.classList.add('active');
            
            // 4. FILTER FOTO
            galleryItems.forEach(item => {
                
                // Ambil kategori foto dari data-category
                const itemCategory = item.getAttribute('data-category');
                
                // Logika filter:
                if (filterValue === 'all') {
                    // Kalau button "Semua" diklik = tampilkan semua foto
                    item.style.display = 'block';
                    
                } else if (itemCategory === filterValue) {
                    // Kalau kategori foto SAMA dengan filter = tampilkan
                    item.style.display = 'block';
                    
                } else {
                    // Kalau kategori foto BEDA = sembunyikan
